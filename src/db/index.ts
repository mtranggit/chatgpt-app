...
export async function createChat(userEmail: string, name: string, msgs: Message[]) {
...
export async function getChatsWithMessages(userEmail: string) : Promise<ChatWithMessages[]> {
	const { rows: chats } = await sql`SELECT * FROM chats WHERE user_email = ${userEmail} ORDER BY timestamp DESC LIMIT 3`;
